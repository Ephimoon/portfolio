...
const CardModal = ({ project, onClose }) => {
  ...
  const techBase = Array.isArray(project.technologies) ? project.technologies : []
  const hasMainImage = typeof project.image === 'string' && project.image.trim().length > 0
  const gallery = Array.isArray(project.moreimages)
    ? project.moreimages.filter((src) => typeof src === 'string' && src.trim().length > 0)
    : []

  const isMobile = typeof window !== 'undefined' && window.innerWidth < 768
  const shouldScroll = isMobile && techBase.length > 5
  const techList = shouldScroll ? [...techBase, ...techBase] : techBase

  return (
    <div className="fixed inset-0 z-50 bg-second/30 flex items-center justify-center" onClick={onClose}>
      <div
        className="relative bg-white w-full max-w-3xl max-h-[100vh] md:max-h-[90vh] md:rounded-3xl shadow-lg flex flex-col overflow-hidden"
      ...
      >
            ...
            <div className="w-full mt-2 px-2 overflow-hidden sm:overflow-visible">
              <div
                className={`flex gap-4 whitespace-nowrap ${
                  shouldScroll
                    ? 'w-max animate-scroll-left-stack'
                    : 'w-full justify-center flex-wrap sm:whitespace-normal sm:animate-none'
                }`}
              >
                {techList.map((tech, i) => (
                  <div key={`${i}-${tech?.tech_name || 'tech'}`} className="relative group shrink-0">
                    <div className="flex flex-col items-center text-center">
                      {tech?.tech_image ? (
                        <img src={tech.tech_image} alt={tech?.tech_name || 'tech'} className="h-6 md:h-8" />
                      ) : (
                        <span className="h-6 md:h-8 flex items-center text-xs text-gray-500">
                          {tech?.tech_name || 'Tech'}
                        </span>
                      )}
        ...
        <div className="overflow-y-auto px-6 py-4 space-y-4">
          {hasMainImage && (
            <img src={project.image} alt={`${project.name} main`} className="w-full rounded-md p-4" />
          )}
